...
type Timeline interface {
	...
	GetHomeTimeline(ctx context.Context, accountID string, page *paging.Page[string], local bool) ([]*gtsmodel.Status, error)

	// GetPublicTimeline fetches the account's PUBLIC timeline -- ie., posts and replies that are public.
	// It will use the given filters and try to return as many statuses as possible up to the limit.
	//
	// Statuses should be returned in descending order of when they were created (newest first).
	GetPublicTimeline(ctx context.Context, page *paging.Page[string], local bool) ([]*gtsmodel.Status, error)
	...
	GetFavedTimeline(ctx context.Context, accountID string, page *paging.Page[string]) ([]*gtsmodel.StatusFave, error)

	// GetListTimeline returns a slice of statuses from followed accounts collected within the list with the given listID.
	// Statuses should be returned in descending order of when they were created (newest first).
	GetListTimeline(ctx context.Context, listID string, page *paging.Page[string]) ([]*gtsmodel.Status, error)

	// GetTagTimeline returns a slice of public-visibility statuses that use the given tagID.
	// Statuses should be returned in descending order of when they were created (newest first).
	GetTagTimeline(ctx context.Context, tagID string, page *paging.Page[string]) ([]*gtsmodel.Status, error)
}
