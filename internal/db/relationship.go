...
type Relationship interface {
	...
	// IsMutualBlocked checks whether there is a block in place between either of account1 and account2.
	IsMutualBlocked(ctx context.Context, accountID1 string, accountID2 string) (bool, error)
	...
	// DeleteFollowsByOriginAccountID
	DeleteFollowsByOriginAccountID(ctx context.Context, originAccountID string) error

	// DeleteFollowsByTargetAccountID
	DeleteFollowsByTargetAccountID(ctx context.Context, targetAccountID string) error
	...
	// DeleteFollowRequestByURI
	DeleteFollowRequestByURI(ctx context.Context, uri string) error

	// DeleteFollowRequestsByOriginAccountID
	DeleteFollowRequestsByOriginAccountID(ctx context.Context, originAccountID string) error

	// DeleteFollowRequestsByTargetAccountID
	DeleteFollowRequestsByTargetAccountID(ctx context.Context, targetAccountID string) error
	...
	// GetAccountFollowedBys fetches follows that target given accountID.
	GetAccountFollowedBys(ctx context.Context, accountID string) ([]*gtsmodel.Follow, Error)

	// GetAccountLocalFollowedBy fetches follows that target given accountID, only including follows from this instance.
	GetAccountLocalFollowedBys(ctx context.Context, accountID string) ([]*gtsmodel.Follow, error)

	// CountAccountFollowedBy returns the amounts that the given ID is followed by.
	CountAccountFollowedBys(ctx context.Context, accountID string) (int, Error)

	// CountAccountLocalFollowedBy returns the amounts that the given ID is followed by, only including follows from this instance.
	CountAccountLocalFollowedBys(ctx context.Context, accountID string) (int, error)
}
